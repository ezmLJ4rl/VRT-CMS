/* A quiet, trustworthy progress primitive for project funding, pledges, and timelines. */
const FILLS = {
...
export default function ProgressBar({ pct, tone = 'offering', height = 'h-3', label, value, hint }) {
    ...
    <div className="min-w-0">
      {(value || label) && (
        <div className="mb-2 flex min-w-0 items-baseline justify-between gap-3">
          ...
          {value && <span className="shrink-0 font-display text-sm font-semibold tabular-nums text-ink-900">{value}</span>}
        </div>
      )}
      <div
        className={`relative w-full overflow-hidden rounded-full border border-ink-200 bg-ink-50 p-0.5 ${height}`}
        role="progressbar"
