 ...
 * `reports.peopleAttend` (a key that only ever existed in the *admin* catalog)
 * and the half-added `common.cancel` shipped here.
 *
...
describe('translation catalogs', () => {
  ...
  it('detects a key that is genuinely absent', () => {
    ...
    expect(resolves(enKeys, 'reports.peopleAttend')).toBe(false);
    expect(resolves(enKeys, 'common.cancel')).toBe(true);
    expect(resolves(swKeys, 'common.cancel')).toBe(true);
  });
